...
@interface HCDownLoader ()<NSURLSessionDataDelegate>
{
    // 临时文件的大小
...
- (void)downLoadWithURL: (NSURL *)url {
    ...
    if ([HCDownLoaderFileTool isFileExists:self.cacheFilePath]) {
        ...
        if (self.downLoadInfo) {
            self.downLoadInfo([HCDownLoaderFileTool fileSizeWithPath:self.cacheFilePath]);
        }
        
        self.state = HCDownLoaderStateSuccess;
    ...
    [self cancel];
    // 2. 读取本地的缓存大小
    _tmpFileSize = [HCDownLoaderFileTool fileSizeWithPath:self.tmpFilePath];
...
- (void)cancelAndClearCache {
    ...
    // 删除缓存
    [HCDownLoaderFileTool removeFileAtPath:self.tmpFilePath];
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler
{
    ...
    if (_tmpFileSize == _totalFileSize) {
        NSLog(@"文件已经下载完成, 移动数据");
        // 移动临时缓存的文件 -> 下载完成的路径
        [HCDownLoaderFileTool moveFile:self.tmpFilePath toPath:self.cacheFilePath];
        self.state = HCDownLoaderStateSuccess;
        // 取消请求
...
- (void)setState:(HCDownLoaderState)state {
    if (_state == state) {
        return;
    }
    _state = state;

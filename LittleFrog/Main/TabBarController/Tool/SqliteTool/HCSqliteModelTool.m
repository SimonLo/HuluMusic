...
#import "HCSqliteModelTool.h"
#import "HCSqliteTool.h"
#import "HCModelTool.h"
#import "HCTableTool.h"


@implementation HCSqliteModelTool
...
+ (BOOL)createTableWithModelClass: (Class)cls withUID: (NSString *)uid {
    ...
    // 1. 判断 表是否存在
    NSString *tableName = [HCModelTool getTableNameWithModelClass:cls];
    BOOL isTableExists = [HCTableTool isTableExists:tableName uid:uid];
    ...
    NSDictionary *ivarNameTypeDic = [HCModelTool getModelIvarNameSqlTypeDic:cls];
    ...
    if (!isTableExists) {
        ...
        return [HCSqliteTool dealSql:createTabelSQL withUID:uid];
    }


    // 表需要更新
    NSString *tmpTableName = [HCModelTool getTempTableNameWithModelClass:cls];
    ...
    NSArray *columnNames = [HCTableTool getTableAllColumnNames:tableName uid:uid];
    NSArray *modelColumnNames = [HCModelTool getModelIvarNames:cls];
    ...
    return [HCSqliteTool dealSqls:updateSqls withUID:uid];
...
+ (NSArray *)queryAllModels:(Class)cls uid: (NSString *)uid {
    ...
    NSArray *rowDicArray = [HCSqliteTool querySql:sql withUID:uid];
...
+ (NSArray *)queryModels:(Class)cls sql: (NSString *)sql uid: (NSString *)uid {
    ...
    NSArray *array = [HCSqliteTool querySql:sql withUID:uid];
...
+ (BOOL)deleteModel: (id)model uid: (NSString *)uid {
    NSString *tableName = [HCModelTool getTableNameWithModelClass:[model class]];
    ...
    return [HCSqliteTool dealSql:sql withUID:uid];
...
+ (BOOL)saveModel: (id)model uid: (NSString *)uid {
    ...
    NSArray *res = [HCSqliteTool querySql:sql withUID:uid];

    // 字段
    NSArray *columnNames = [HCModelTool getModelIvarNames:[model class]];
    ...
    if (res.count > 0) {
        ...
        return [HCSqliteTool dealSql:insertSql withUID:uid];
    }
...
+ (NSArray *)parseRowDicArray: (NSArray *)rowDicArray withModelClass: (Class)cls {
    ...
    // 模型真正的字段 - > 类型
    NSDictionary *modelColumnNames = [HCModelTool getModelIvarNameIvarTypeDic:cls];
...
+ (BOOL)isRequireUpdate:(Class)cls uid: (NSString *)uid {
    ...
    NSArray *tableColumnNames = [HCTableTool getTableAllColumnNames:[HCModelTool getTableNameWithModelClass:cls] uid:uid];
...
+ (NSDictionary *)naoTypeSQLRelation {
    ...
    return @{
             @(HCSqliteModelToolNAONot) : @"not",
             @(HCSqliteModelToolNAOAnd) : @"and",
             @(HCSqliteModelToolNAOOr) : @"or"
             };
